...
async function login(req, res) {
  ...
  try {
    ...
    logger.error(error);
    res.status(error.statusCode || 500).jsonp(error.message);
  }
}
...
async function register(req, res) {
  ...
  try {
    ...
    res.set("Cache-Control", "private, max-age=5");
    res.status(201).jsonp(user.package());
  }
  catch(error) {
    logger.error(error);
    res.status(error.statusCode || 500).jsonp(error.message);
...
async function store(req, res) {
  ...
  try {
    ...
    let results = await junction.recall({userid: userid});
    if (results.result !== 'ok')
      throw StorageError({statusCode: 401}, "invalid userid/password");
    ...
    // return user's record
    res.set("Cache-Control", "private, max-age=5");
    res.status(200).jsonp(user.package());
  }
  catch(error) {
    logger.error(error);
    res.status(error.statusCode || 500).jsonp(error.message);
