...
async function test(req, res) {
  ...
  var j1, j2;
  try {
    j1 = storage.activate("csv|./test/data/|testfile.csv|*", { filename: './test/data/testfile.csv' });
    j2 = storage.activate("csv|./test/data/|testoutput.csv|*", { filename: './test/data/testoutput.csv' });
...
async function transfer(req, res) {
  ...
  var j1, j2;
  try {
    j1 = storage.activate(config.smt[source.smt], source.options);
    j2 = storage.activate(config.smt[destination.smt], destination.options);
...
async function importFile(req, res) {
  ...
  form.parse(req, async function(err, fields, files) {
    ...
    for (let i = 0; i < importList.length; i++) {
      ...
      var j1, j2;
      try {
        j1 = storage.activate("csv|.|input.csv|*", { filename: importList[i].filename });
        j2 = storage.activate("elasticsearch|./test/data/|testoutput.csv|*");
