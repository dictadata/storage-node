...
async function uploadFiles(req, res) {
  ...
  form.parse(req, async function (err, fields, files) {
    ...
    for (let i = 0; i < importList.length; i++) {
      ...
      var jo, jt;
      try {
        jo = await storage.activate("csv|.|input.csv|*", { filename: importList[i].filename });
        jt = await storage.activate("elasticsearch|./test/data/|testoutput.csv|*");

        var reader = jo.getReadStream();
        var writer = jt.getWriteStream();
