...
exports.startup = async (config) => {
  ...
  try {
    ...
    const accountsEncoding = JSON.parse(await fs.readFile(path.join(__dirname, 'accounts_encoding.json')));
...
var recall = exports.recall = async function (userid) {
  ...
  if (results.result === 'ok') {
    account = new Account();
    if (junction.engram.keyof === 'key')
      account.decode(results.data[userid]);
    else
      account.decode(results.data[0]);
  }
...
var retrieve = exports.retrieve = async function (pattern) {
  ...
  if (results.result === 'ok') {
    ...
    account.decode(results.data[keys[0]]);
  }
...
var store = exports.store = async function (account) {
  ...
  let pattern = {};
  if (junction.engram.keyof === 'key') {
    construct = account.encode(false);
    pattern["key"] = account.userid;
  }
  else {
    construct = account.encode(true);
