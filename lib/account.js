...
module.exports = class Account {
  ...
  /**
   * Create flat representation of construct for storage.
   * 
   * @param {*} pack 
   */
  encode(pack = false) {
    let construct = {
      ...
      roles:       pack ? JSON.stringify(this.roles) : this.roles,
      dateCreated: this.dateCreated,
      dateUpdated: this.dateUpdated,
      lastLogin:   this.lastLogin,
      profile:  pack ? JSON.stringify(this.profile) : this.profile,
      settings: pack ? JSON.stringify(this.settings) : this.settings
    }
    return construct;
  }

  /**
   * Parse storage construct into the account properties.
   * Parses flattend construct or normal json.
   * 
   * @param {*} construct 
   */
  decode(construct) {
    this.userid = construct.userid;
    this.password = construct.password;
    this.roles = (typeof construct.roles === 'string') ? JSON.parse(construct.roles) : construct.roles;
    this.dateCreated = construct.dateCreated;
    this.dateUpdated = construct.dateUpdated;
    this.lastLogin = construct.lastLogin;
    this.profile = (typeof construct.profile === 'string') ? JSON.parse(construct.profile) : construct.profile;
    this.settings = (typeof construct.settings === 'string') ? JSON.parse(construct.settings) : construct.settings;
  }
