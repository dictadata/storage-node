...
exports.startup = async (config) => {
  ...
  logger.info("accounts SMT: " + config.accounts_smt);

  let engram = new storage.Engram(config.accounts_smt);
  if (engram.keyof !== 'key') {
    throw new StorageError({statusCode: 400}, "invalid key type in config.accounts_smt");
  }
