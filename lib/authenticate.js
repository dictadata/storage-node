...
exports.startup = async (config) => {
  ...
  try {
    ...
    if (encoding)
      logger.info("accounts schema valid: " + junction.engram.smt.schema);
    else
    ...
    logger.error('authenticate startup failed: ', err);
  }
  finally {
    if (junction)
      junction.relax();
 ...
 */
let hashPwd = exports.hashPwd = function (pwd) {
