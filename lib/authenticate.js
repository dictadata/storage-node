 ...
 * @param {*} userid
 */
let getAccount = exports.getAccount = async function (userid) {
 ...
 */
let findAccount = exports.findAccount = async function (pattern) {
 ...
 * @param {*} account
 */
let createAccount = exports.createAccount = async function (account) {
  ...
  let results = await junction.store(account, {userid: account.userid});
  if (results.result !== 'ok')
...
exports.basic = async function (userid, password, done) {
  ...
  try {
    let publicAccount = new Account(userid);
    publicAccount.password = password;
    publicAccount.roles = _defaultRoles;

    let account = await getAccount(userid);
    if (!account)
      return done(null, publicAccount);  // not found, public account

    let ep = hashPwd(password);
    if (account.password !== ep)
      return done(null, publicAccount);  // bad password, public account
